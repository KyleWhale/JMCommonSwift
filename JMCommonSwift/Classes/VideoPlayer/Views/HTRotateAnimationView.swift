...
class HTRotateAnimationViewConfigure: NSObject {
    /// 开始起点
    ...
    fileprivate class func defaultConfigure() -> HTRotateAnimationViewConfigure {
        let configure = HTRotateAnimationViewConfigure()
        return configure
    }
}

class HTRotateAnimationView: UIView {
    /// 默认配置
    private let configure = HTRotateAnimationViewConfigure.defaultConfigure()
    /// layer数组
    ...
    /// 更新配置
    func updateWithConfigure(_ configureBlock: ((HTRotateAnimationViewConfigure) -> Void)?) {
...
extension HTRotateAnimationView {
    /// 开始动画
    func startAnimation() {
