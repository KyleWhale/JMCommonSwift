...
public extension UIView {
     ...
     */
    func yn_makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func yn_makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
    ...
    func yn_makeToastEnum(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, point: ToastPosition = ToastPosition.center, title: String?, image: UIImage?, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
            let point = point.centerPoint(forToast: toast, inSuperview: self)
            showToast(toast, duration: duration, point: point, completion: completion)
        } catch ToastError.missingParameters {
            print("Error: message, title, and image cannot all be nil")
        } catch {}
    ...
    /**
     ...
     @param position The toast's position
     */
    func yn_makeToastActivity(_ position: ToastPosition,message:String? = nil) {
        // sanity
    ...
    /**
     ...
     @param point The toast's center point
     */
    func makeToastActivity(_ point: CGPoint ,message:String? = nil) {
        // sanity
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView == nil else { return }
    ...
    private func createToastActivityView(_ message:String?) -> UIView {
        ...
        tostView.yn_cornerRadius = 5
        activityView.addSubview(tostView)
        
        let activity = UIActivityIndicatorView(style: UIActivityIndicatorView.Style.whiteLarge)
        activity.yn_size = CGSize(width: style.activitySize.width, height: style.activitySize.height)
        activity.startAnimating()
        tostView.yn_width = style.activitySize.width + 60
        tostView.yn_height = style.activitySize.height + 60
        tostView.center = CGPoint(x: activityView.yn_width/2, y: activityView.yn_height/2)
        activity.center = CGPoint(x: tostView.yn_width/2, y:tostView.yn_height/2)
        tostView.addSubview(activity)
        ...
        if let msg = message {
            ...
            tostView.yn_height += (messageLabel.yn_height)
            tostView.yn_width = (messageLabel.yn_width+20*2)
            tostView.center = CGPoint(x: activityView.yn_width/2, y: activityView.yn_height/2)
            activity.yn_top = 30
            activity.yn_centerX = tostView.yn_width/2
            messageLabel.yn_top = activity.yn_bottom + padding
            messageLabel.yn_centerX = activity.yn_centerX
    ...
    func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        ...
        if let image = image {
            ...
            imageView?.frame = CGRect(x: x, y: y, width: style.imageSize.width, height: style.imageSize.height)
        }
        ...
        if let messageLabel = messageLabel {
            ...
            messageRect.origin.y = imageView == nil ? 20 : imageRect.origin.y + imageRect.size.height + 12
            messageRect.size.width = messageLabel.bounds.size.width
            messageRect.size.height = messageLabel.bounds.size.height
        }
        
        let longerWidth = max(imageRect.size.width, messageRect.size.width)
        let wrapperWidth = max(117, messageRect.size.width + 20)
        let wrapperHeight =  imageRect.origin.y + imageRect.size.height + (imageView == nil ? style.verticalPadding : 12) + style.verticalPadding  + messageRect.size.height
        ...
        if let imageView = imageView {
            imageView.yn_centerX = wrapperView.yn_width/2
        ...
        if let messageLabel = messageLabel {
            ...
            messageLabel.yn_centerX = imageView?.yn_centerX ?? wrapperView.yn_width/2
        }
...
public struct ToastStyle {
     ...
     */
    public var imageSize = CGSize(width: 48.0, height: 48.0)
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        let topPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview.bcSafeAreaInsets.top
        let bottomPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview.bcSafeAreaInsets.bottom
...
fileprivate extension UIView {
    ...
    var bcSafeAreaInsets: UIEdgeInsets {
...
public extension UIView{
    ...
    enum YNToastType : String {
        case success = "toastSuccess"
        case failure = "toastFail"
    }
    
    private struct AssociatedKeys {
        static var descriptiveName = "AssociatedKeys.DescriptiveName.loadingView"
    }
    
    ///展示Toast
    func yn_showToast(_ message:String? , _ image:YNToastType? = nil) {
        yn_hideToastActivity()
        if let image = image {
            yn_makeToast(message, point: CGPoint(x:self.yn_width/2,y:self.yn_height/2), title: nil, image: UIImage(named: image.rawValue), completion: nil)
        }else {
            yn_makeToast(message, point: CGPoint(x:self.yn_width/2,y:self.yn_height/2), title: nil, image: nil, completion: nil)
        }
    }
    
    ///展示Toast
    func yn_showToast(_ message:String? , _ duration:TimeInterval, _ image:YNToastType? = nil) {
        yn_hideToastActivity()
        if let image = image {
            yn_makeToast(message,duration: duration, point: CGPoint(x:self.yn_width/2,y:self.yn_height/2), title: nil, image: UIImage(named: image.rawValue) , completion: nil)
        }else {
            yn_makeToast(message,duration: duration, point: CGPoint(x:self.yn_width/2,y:self.yn_height/2), title: nil, image: nil , completion: nil)
        }
        
    }
    ///展示Toast
    func yn_showToast(_ message:String? , _ image:YNToastType? = nil , point : ToastPosition = ToastPosition.center) {
        yn_hideToastActivity()
        if let image = image {
            yn_makeToastEnum(message, point: point, title: nil, image: UIImage(named: image.rawValue), completion: nil)
        }else {
            yn_makeToastEnum(message, point: point, title: nil, image: nil, completion: nil)
        }
